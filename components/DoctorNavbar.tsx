...
import { LayoutDashboard, Users, LogOut, Menu, X, KeyRound } from "lucide-react"
...
export function DoctorNavbar() {
  ...
  const navItems = [
    ...
    { name: "Patient Access Code", href: "/doctor/access", icon: KeyRound },
  ]
        ...
        <div className="md:hidden fixed inset-x-0 top-16 bottom-0 z-50 bg-background/95 backdrop-blur-xl border-t border-border p-6 flex flex-col justify-between shadow-2xl animate-in slide-in-from-top-2">
          <nav className="flex flex-col space-y-4">
            <div className="pb-3 border-b border-border">
              <p className="text-xs text-emerald-600 uppercase font-semibold">Doctor Portal</p>
              <p className="text-lg font-bold text-foreground">Clinical Workstation</p>
            </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className="mr-3 h-5 w-5 text-emerald-600" />
                  {item.name}
                </Link>
              )
            })}
