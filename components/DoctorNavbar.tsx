        ...
        <div className="md:hidden fixed inset-x-0 top-16 bottom-0 z-[999] bg-white dark:bg-slate-950 border-t border-border p-6 flex flex-col justify-between shadow-2xl animate-in slide-in-from-top-2 overflow-y-auto">
          <nav className="flex flex-col space-y-3">
            <div className="pb-4 mb-2 border-b border-border/60">
              <p className="text-xs text-emerald-600 uppercase font-bold tracking-wider">Doctor Portal</p>
              <p className="text-xl font-extrabold text-foreground mt-0.5">Clinical Workstation</p>
            </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={`mr-3 h-5 w-5 ${isActive ? "text-emerald-600" : "text-muted-foreground"}`} />
                  {item.name}
                </Link>
              )
            })}
